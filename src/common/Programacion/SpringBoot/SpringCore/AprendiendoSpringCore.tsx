      ...
      >
        ...
        >
          <div className="flex items-center justify-center space-x-4 mb-4">
            <FaCodeBranch className="text-6xl text-green-500" />
            <h1 className="text-5xl font-bold text-white">Aprendiendo Spring Core</h1>
          </div>
          <p className="text-gray-300 text-xl max-w-4xl mx-auto">
            Spring Core es el núcleo del framework Spring. Permite construir aplicaciones Java 
            desacopladas y organizadas a través de principios como la inversión de control (IoC) y 
            la inyección de dependencias. En esta sección aprenderás desde cero cómo funcionan los 
            componentes, beans, configuraciones y servicios utilizando anotaciones y el contenedor de Spring.
          </p>
        </motion.div>
          ...
          >
            <div className="flex items-center space-x-3 mb-4">
              ...
              <h2 className="text-2xl font-semibold text-white">
                {ejemplosSpringCoreBasico[ejemploActual].titulo}
              </h2>
            </div>
            <p className="text-gray-300 mb-4">
              {ejemplosSpringCoreBasico[ejemploActual].descripcion}
            </p>
            <pre className="bg-gray-900/80 p-6 rounded-lg overflow-x-auto text-sm text-white font-mono border border-purple-500/20">
              {ejemplosSpringCoreBasico[ejemploActual].codigo}
            </pre>
          </motion.div>
          ...
          >
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-purple-500/30">
              ...
              <div className="bg-white/5 rounded-lg p-6 border border-purple-500/20">
                <p className="text-white text-lg">{ejemplosSpringCoreBasico[ejemploActual].resultado}</p>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-purple-500/30">
              ...
              <p className="text-gray-300 whitespace-pre-line">
                {ejemplosSpringCoreBasico[ejemploActual].explicacion}
              </p>
            </div>
          </motion.div>
        </div>
          ...
          <button
            onClick={() =>
              setEjemploActual((prev) =>
                Math.min(ejemplosSpringCoreBasico.length - 1, prev + 1)
              )
            }
            disabled={ejemploActual === ejemplosSpringCoreBasico.length - 1}
            className={`px-6 py-2 rounded-lg font-semibold transition ${
              ejemploActual === ejemplosSpringCoreBasico.length - 1
                ? "bg-gray-600 cursor-not-allowed text-gray-300"
                : "bg-purple-600 hover:bg-purple-700 text-white"
            }`}
          >
